...
if ($action === "check_availability") {
    ...
    $query = "SELECT * FROM reservations" . ($id ? " WHERE id = '$id'" : "");
    $result = $mysqli->query($query);

    if (!$result) {
        echo json_encode(["error" => "Error fetching reservations: " . $mysqli->error]);
        exit;
    }
    ...
    $id = isset($input['id']) ? $mysqli->real_escape_string($input['id']) : '';
    ...
    if (!$id || !$lab || !$fecha || !$hora_inicio || !$hora_fin || !$teacher_name || !$practice_type || !$materials || $num_students <= 0) {
        echo json_encode(["success" => false, "message" => "Missing or invalid parameters"]);
        exit;
    }

    if (is_non_working_day($fecha, $holidays_2025)) {
        echo json_encode(["success" => false, "message" => "Cannot update on weekends or holidays"]);
        exit;
    }

    $checkQuery = "SELECT * FROM reservations WHERE laboratorio = '$lab' AND fecha = '$fecha' 
    ...
    if ($checkResult->num_rows > 0) {
        echo json_encode(["success" => false, "message" => "Time slot already reserved"]);
        exit;
    }

    $updateQuery = "UPDATE reservations SET laboratorio = '$lab', fecha = '$fecha', hora_inicio = '$hora_inicio', 
                    hora_fin = '$hora_fin', teacher_name = '$teacher_name', practice_type = '$practice_type', 
                    materials = '$materials', num_students = $num_students WHERE id = '$id'";
    error_log("Executing update query: $updateQuery");

    if ($mysqli->query($updateQuery) === TRUE) {
        error_log("Reservation updated for id $id");
        echo json_encode(["success" => true, "message" => "Reservation updated"]);
    } else {
        error_log("Error updating reservation: " . $mysqli->error);
        echo json_encode(["success" => false, "message" => "Error updating reservation: " . $mysqli->error]);
    }
